...
def task_12_list_suppliers_from_specified_countries(cur):
    ...
    Returns: 8 records
    """
    cur.execute('''SELECT SupplierID, SupplierName, ContactName, City, Country FROM Suppliers Where Country
                   IN ('USA', 'UK', 'Japan') ''')
    return cur.fetchall()
...
def task_13_list_products_from_sweden_suppliers(cur):
    ...
    Returns: 3 records
    """
    cur.execute('''SELECT ProductName FROM Products INNER JOIN Suppliers ON 
...
def task_16_match_all_customers_and_suppliers_by_country(cur):
    ...
    Returns: 194 records
    """
    cur.execute('''SELECT CustomerName, Customers.Address, Customers.Country AS CustomerCountry, Suppliers.Country
                   AS SupplierCountry, SupplierName
                   FROM Customers 
                   FULL JOIN Suppliers ON Suppliers.Country = Customers.Country
